...
namespace CombineAndMinify
{
	public class HeadAdapter : ControlAdapter
	{
		...
		protected override void Render(HtmlTextWriter writer)
		{
			...
			{
				string headCacheKey = HeadCacheKey(HttpContext.Current.Request.Url, currentCounter, cs.HeadCaching);
		...
		private void ProcessAllImages(ControlCollection cc, UrlProcessor urlProcessor, bool removeWhitespace)
		{
			...
			foreach (Control c in cc)
			{
					...
					if (!string.IsNullOrEmpty(literalContent))
					{
						if (imagesNeedProcessing)
						{
							...
							while (m.Success)
							{
								...
								string newUrl = urlProcessor.ProcessedUrl(oldUrl, FileTypeUtilities.FuzzyFileType.Image, false, Control.Page.Request.Url, null);
				...
				else if ((c is HtmlImage) && imagesNeedProcessing)
				{
					...
					hi.Src = urlProcessor.ProcessedUrl(hi.Src, FileTypeUtilities.FuzzyFileType.Image, false, Control.Page.Request.Url, versionId);
				}
				else if ((c is HyperLink) && imagesNeedProcessing)
				{
					...
					if (!string.IsNullOrEmpty(hl.ImageUrl))
					{
						...
						hl.ImageUrl = urlProcessor.ProcessedUrl(hl.ImageUrl, FileTypeUtilities.FuzzyFileType.Image, false, Control.Page.Request.Url, versionId);
					}
				}
				else if ((c is Image) && imagesNeedProcessing)
				{
					...
					i.ImageUrl = urlProcessor.ProcessedUrl(i.ImageUrl, FileTypeUtilities.FuzzyFileType.Image, false, Control.Page.Request.Url, versionId);
				}
				else
				{
		...
		private string HeadCacheKey(Uri currentUrl, int counter, ConfigSection.HeadCachingOption headCachingOption)
		{
			...
			switch (headCachingOption)
			{
				...
				case ConfigSection.HeadCachingOption.PerFolder:
					...
					if (idxLastSlash == -1)
					{
						return keyPrefix + counter + currentUrl.ToString();
					}

					return keyPrefix + counter + currentUrl.AbsoluteUri.Substring(0, idxLastSlash);

				case ConfigSection.HeadCachingOption.PerPage:
					// Key is based on folder + file, but not on query string
					return keyPrefix + counter + currentUrl.AbsolutePath;
