	...
	public class CombinedFile
	{
			...
			out FileTypeUtilities.FileType fileType)
		{
			...
			if (cacheElement == null)
			{
				...
				string combinedContent = Filter.Execute(combinedContentSb.ToString(), fileType);
				if (!string.IsNullOrEmpty(combinedContent))
				{
					...
					else if(processingJavaScriptFile)
					{
						...
						cacheElement.CombinedFileContent = jsminCs.Minify(combinedContent, processingJavaScriptFile);
					}
			...
			ref string fileContent, Uri fileUrl, UrlProcessor urlProcessor)
		{
			...
			while (m.Success)
			{
				...
				if (urlProperties.Count > 0)
				{
					...
					string absoluteUrl = urlProcessor.ProcessedUrl(relativeUrl, FileTypeUtilities.FuzzyFileType.ImageOrFont, true, fileUrl, null);
		...
		{
			if (FileTypeUtilities.IsInlinedImage(path)) { return ""; }
